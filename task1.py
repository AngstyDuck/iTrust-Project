...
Note about dataset:
- ER1 is unused for the entire duration. For water level,
...
class Task1:
            ...
            with open(DIRSTATESWADI, "w+") as csvfile1:
                ...
                for row in spamreader:
                    if counter0 == 0:
                        ...
                        # FR8
                        currentRow[5] = int(row[indexList["2_MV_001"]])
    ...
    def testFR2LogExp(self):
        """
        Test validity of axiomatic matrix on the expression of FR2. This is done
        by counting number of FR2 state change that coincides with changes in
        FRs other than those in dptVarIndex.

        [FR1, FR2, FR3, FR6, FR7, FR8]

        Result:
        counter0: 1206854; counter1: 6730, percentage: 0.005576482325119691
        """
        indptVarIndex = 1
        dptVarIndex = [0,1]
        counter0 = 0
        ...
        with open(DIRSTATESWADI) as csvfile:
            spamreader = csv.reader(csvfile, delimiter=" ", quotechar="|")

            for row in spamreader:
                if counter0 > 0:

                    # if current row and previous row is not invalid
                    if row[0] != -1 and prevRow[0] != -1:

                        # if there is a change in FR2
                        if prevRow[1] != row[1]:

                            # if there is change in dependent variables other than dptVarIndex
                            for i in range(len(row)):
                                if i not in dptVarIndex:
                                    if prevRow[i] != row[i]:
                                        counter1 += 1
            ...
            print("counter0: {0}; counter1: {1}, percentage: {2}".format(counter0, counter1, counter1/counter0))
